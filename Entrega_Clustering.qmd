...
Con el método de la silueta, nos sugiere escoger $k=2$, en cambio, con el siguiente, nos sugiere $8$ clústers. Como no hemos salido de dudas, realicemos clústers para $k =2,4,8$ 
...
Encontramos codos en $k=2$, $k=4$ y uno muy pronunciado en $k=6$. Para ver cual sería óptimo, como en el caso anterior, visualicemos otros métodos como el de la *silueta* y *gap_stat*.
...
Este método combina los dos anteriores. dividiendo a partes iguales el conjunto de observaciones y aplicando PAM a cada una de las partes para identificar los *medoids*. Calcular la suma total de las distancias entre cada observación y su correspondiente medoid y seleccionar como clustering final aquel que ha conseguido menor suma total de distancias intra-clusters. 

Como en los casos anteirores han salido que $k=2, 4$ eran los más óptimos, realicemos *Clara* con estos valores de $k$:
...
Comprobemos si la clusterizacion en $4$ grupos es semejante a la de la variable `stage`
...
table(x = tabla_cluster[,c(2,3)])
```
Observemos que el método *Clara* con $k=4$ tampoco clusteriza en función del estado de cirrosis en que se encuentra el paciente.
...
Efectivamente, viendo el gráfico y utilizando el criterio del codo, nos quedaremos con $2$ componentes principales.
